...
export class Flights /* implements OnInit */ {
  ...
  private store = inject(FlightsStore);
  ...
  flights = this.store.displayedFlights;
  totalItems = this.store.totalItems;
  filters = this.store.filters;
  page = this.store.page;
  totalPages = () => Math.ceil(this.store.totalItems() / this.store.pageSize());
  ...
  onFilter(field: FlightFilterKey, inputVal: string) {
    this.store.filters.update((f) => ({ ...f, [field]: inputVal })); // Ex. this.filters.arrival_city = "Stockholm".
  }
